...
"""This is the code for forming guided swarm clusters
with hard coded obstacle positions"""
        ...
        if (self.dis_err) >= 0.75:
            ...
            for i,z in enumerate(self.disij):
                if i not in excluded_bot:
                    if (min(self.ranges[0:deg]) <= dst or min(self.ranges[(359-deg):]) <= dst or wall_distance <= wall_radius) and z >= 0.65: 
                        self.wall_following()
            ...
            if len(self.neigh) < 2 or wall_distance < 2*wall_radius: 
                self.goal = Point(3.0, -2.0, 0.0)
                self.wall_following()
